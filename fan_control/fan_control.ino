...
// Abbreviated WiFi quality for the 20-col LCD (max 5 chars)
String wifiQualityShort() {
    if (provisioningMode) return "AP";
    if (WiFi.status() != WL_CONNECTED) return "Offl";
    int rssi = WiFi.RSSI();
    if (rssi >= -55) return "Exlt";
    if (rssi >= -67) return "Good";
    if (rssi >= -75) return "Fair";
    return "Weak";
}
...
//
//  Fixed layout (20 columns x 4 rows):
//  Row 0  T: 28.5°C  H:  65%    ← sensor readings
//  Row 1  Fan:ON   Sw:AUTO       ← fan state + switch
//  Row 2  On:28.0°C Off:26.0°C   ← ON/OFF thresholds
//  Row 3  > RUNNING               ← state or error (highest-priority)
// ============================================
void initLocalDisplay() {
...
void updateLocalDisplay() {
    ...
    // Format: "T: 28.5°C  H: 65%   "  (20 chars)
    // Degree symbol is char(0xDF) on HD44780
    String line0;
    if (sensorStatus == "ok" || sensorStatus == "stale") {
        snprintf(buf, sizeof(buf), "T:%5.1f%cC  H:%3.0f%%   ",
                 currentTemp, (char)0xDF, currentHumidity);
        line0 = String(buf);
    } else {
        // "SENSOR: FAILED     " or "SENSOR: UNKNOWN    "
        String s = sensorStatus;
        s.toUpperCase();
        line0 = "SENSOR: " + s;
    }

    // --- Row 1: Fan State & Physical Switch ---
    // Format: "Fan:ON   Sw:AUTO    "  (20 chars)
    snprintf(buf, sizeof(buf), "Fan:%-3s  Sw:%-4s    ",
             fanOn ? "ON" : "OFF",
             physicalSwitchState.c_str());
    String line1 = String(buf);

    // --- Row 2: cycles between thresholds and IP/WiFi signal ---
    // Page A (4 s): "On:28.0°C Off:26.0°C"  (thresholds)
    // Page B (4 s): "192.168.1.100   Good " (IP left-15, quality right-5)
    static bool lcdRow2ShowNetwork = false;
    static unsigned long lastLcdRow2Toggle = 0;
    {
        unsigned long nowMs = millis();
        if (nowMs - lastLcdRow2Toggle >= LCD_ROW2_INTERVAL_MS) {
            lcdRow2ShowNetwork = !lcdRow2ShowNetwork;
            lastLcdRow2Toggle = nowMs;
        }
    }

    String line2;
    if (lcdRow2ShowNetwork) {
        String ipStr = provisioningMode
            ? WiFi.softAPIP().toString()
            : WiFi.localIP().toString();
        String qual = wifiQualityShort();
        snprintf(buf, sizeof(buf), "%-15s%5s", ipStr.c_str(), qual.c_str());
        line2 = String(buf);
    } else {
        snprintf(buf, sizeof(buf), "On:%4.1f%cC Off:%4.1f%cC",
                 threshold, (char)0xDF,
                 threshold - hysteresis, (char)0xDF);
        line2 = String(buf);
    }

    // --- Row 3: Status / Error (highest priority first) ---
    String line3;
    if (contactorFault) {
        line3 = "! CONTACTOR FAULT";
    } else if (tempAlarm) {
        line3 = "! HIGH TEMP ALARM";
    } else if (sensorFailSafeActive) {
        line3 = "! SENSOR FAIL-SAFE";
    } else if (cooldownActive) {
        unsigned long elapsed = millis() - cooldownStart;
        ...
        switch (controllerState) {
            case STATE_RUNNING:       line3 = "> RUNNING";       break;
            case STATE_AUTO_WAIT:     line3 = "  AUTO: WAITING"; break;
            case STATE_OFF:           line3 = "  FAN OFF";       break;
            case STATE_MIN_RUN_LOCK:  line3 = "  MIN RUN LOCK";  break;
            case STATE_MIN_STOP_LOCK: line3 = "  MIN STOP LOCK"; break;
            case STATE_COOLDOWN:      line3 = "* COOLDOWN";      break;
            case STATE_FAULT:         line3 = "! FAULT";         break;
            default:                  line3 = "  " + String(controllerStateName()); break;
        }
    }
